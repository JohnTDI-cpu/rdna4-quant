...
## File Structure (Published Repository)

The open-source release includes the best-performing INT4 pipeline:

```
rdna4-quant/
  # Documentation
  README.md                    — Main documentation with benchmarks
  RESEARCH_NOTES.md            — This file (comprehensive research summary)
  REPRODUCTION_RECIPE.md       — Step-by-step quantization reproduction guide
  NOVELTY_ANALYSIS.md          — What's new vs existing literature
  DISCOVERY_GFX12_WMMA_OUTPUT.md — WMMA lane mapping guide for RDNA4
  PUBLISH_GUIDE.md             — Publication strategy

  # Inference
  int4_engine_v5.py            — Main engine (HIP C++ decode + rocBLAS prefill)
  ...
  int4_quant_v2.py             — Core INT4 asymmetric + GPTQ algorithms
  ...
  # HIP C++ kernels
  hip_int4/int4_decode_step.hip — INT4 GEMV + attention + norms (~3600 lines)
  hip_int4/setup.py            — Build script (auto-detects GPU arch)

  # Tests & data
  tests/test_kernels.py        — 6 unit tests (quantization + HIP GEMV)
  quality_results_250.json     — ARC/MMLU benchmark results
  sensitivity_map.json         — Layer sensitivity rankings
```

Note: Many experimental scripts (MXFP4, NVFP4, learned rotation, benchmark scripts)
were developed during research but are not included in the release. See the experiment
history above for the full research progression.
...
# Run inference
python3 int4_engine_v5.py --chat
