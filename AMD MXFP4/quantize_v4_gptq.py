"""
Quantization v4 — GPTQ with PROPER calibration for Qwen3-14B.

Key improvements over v2:
  1. C4 dataset for calibration (diverse, unlimited data — not WikiText-2 which has only 37 long samples)
  2. 256 samples × 256 tokens = well-conditioned Hessian (rank 256 for dim 5120)
  3. Asymmetric INT4 + Hadamard rotation + GPTQ
  4. Mixed INT4/INT8 from sensitivity map

v2 had rank-deficient Hessian → GPTQ injected noise → HURT PPL by ~0.1.
v3 (no-GPTQ) proved simple block-quant is better with bad calibration.
v4 tests whether proper calibration FIXES GPTQ.

Usage:
  python quantize_v4_gptq.py --save-dir quantized_v4_gptq
"""
import torch
...
from int8_quant import gptq_quantize_int8
...
def main():
    ...
    parser.add_argument('--save-dir', type=str, default='quantized_v4_gptq')
    parser.add_argument('--sensitivity-map', type=str,
                        default=str(Path(__file__).parent / 'sensitivity_map.json'))
    parser.add_argument('--nsamples', type=int, default=256)
    parser.add_argument('--block-size', type=int, default=32)
    parser.add_argument('--cal-seqlen', type=int, default=256)
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--int8-pct', type=int, default=20,
                        help='Percent of weight groups to keep in INT8')
    ...
    # Load sensitivity map and build precision map
    with open(args.sensitivity_map) as f:
        sens_data = json.load(f)

    ranking = sens_data['ranking']
    total_groups = len(ranking)
    n_int8 = max(1, total_groups * args.int8_pct // 100)
    int8_set = set()
    for i, (li, group, score) in enumerate(ranking[:n_int8]):
        int8_set.add((li, group))

    precision_map = {}
    for li_idx in range(40):
        precision_map[str(li_idx)] = {}
        for g in ['qkv', 'o', 'gate_up', 'down']:
            precision_map[str(li_idx)][g] = 'int8' if (li_idx, g) in int8_set else 'int4'
    ...
    n_i4 = sum(1 for li in precision_map for g in precision_map[li] if precision_map[li][g] == 'int4')
    n_i8 = sum(1 for li in precision_map for g in precision_map[li] if precision_map[li][g] == 'int8')
    rot_str = "Hadamard" if use_rotation else "NoRotation"
    quant_str = "Asymmetric" if use_asymmetric else "Symmetric"
    print(f"\nQuantize v4: {quant_str} INT4 + GPTQ (proper cal) + {rot_str} (block={block_size})")
    print(f"  INT4 groups: {n_i4}, INT8 groups: {n_i8}")
    ...
    t_total = time.time()
    cnt_int4 = 0; cnt_int8 = 0

    for li in range(num_layers):
        layer_file = save_dir / f"layer_{li:03d}.pt"
        layer_prec = precision_map.get(str(li), {})
        ...
        t0 = time.time()
        prec_labels = [f"{g}={'INT8' if layer_prec.get(g, 'int4') == 'int8' else 'INT4a'}"
                       for g in ['qkv', 'o', 'gate_up', 'down']]
        print(f"\nLayer {li+1}/{num_layers}: {', '.join(prec_labels)}")
        ...
        for save_key, proj_names, hessian_name in weight_configs:
            ...
            N_out, K_in = W.shape
            prec = layer_prec.get(save_key, 'int4')
            ...
            # GPTQ quantization
            if prec == 'int8':
                W_int8, W_scales, K_final = gptq_quantize_int8(
                    W_rot.half(), H_rot, block_size=block_size, device=device)
                print(f"  {save_key:8s}: INT8+Had+GPTQ [{N_out}x{K_final}]")
                layer_data[f'{save_key}_weights'] = W_int8.cpu()
                layer_data[f'{save_key}_scales'] = W_scales.cpu()
                layer_data[f'{save_key}_precision'] = 'int8'
                cnt_int8 += 1
            else:
    ...
    meta = {
        ...
        'method': f"v4_{'asym' if use_asymmetric else 'sym'}_{'had' if use_rotation else 'norot'}_gptq_c4cal",
        'block_size': block_size,
        'quant_format': f"mixed_{'int4a' if use_asymmetric else 'int4'}_{'' if use_rotation else 'no'}rot_int8",
        'use_rotation': use_rotation,
        'use_asymmetric': use_asymmetric,
        'precision_map': precision_map,
        'n_int4_groups': cnt_int4, 'n_int8_groups': cnt_int8,
    ...
    print(f"INT4a: {cnt_int4}, INT8: {cnt_int8}")
